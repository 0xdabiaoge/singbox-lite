...
function check_and_install_deps() {
    ...
    if command -v apt-get &>/dev/null; then
        ...
        if [ -n "$pkgs_to_install" ]; then
            ...
            if ! (apt-get update && apt-get install -y $pkgs_to_install); then
                echo -e "${RED}依赖安装失败，请手动执行 'apt-get install -y $pkgs_to_install' 后重试。${NC}"
                exit 1
            fi
        fi
    else
        echo -e "${YELLOW}警告: 未检测到 apt, 无法自动安装依赖。请确保已安装: ${required_pkgs}${NC}"
    fi
    
    if ! command -v yq &>/dev/null; then
        echo "正在安装 yq (用于YAML处理)..."
        wget -qO ${YQ_BINARY} https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64
        if [ $? -ne 0 ]; then
...
function manage_nodes() {
    ...
    local type=$(jq -r ".inbounds[${index}].type" "$CONFIG_FILE")
    local port=$(jq -r ".inbounds[${index}].listen_port" "$CONFIG_FILE")
    local name="${type}-in-${port}"; if [ "$type" == "vless" ]; then name="vless-reality-${port}"; elif [ "$type" == "shadowsocks" ]; then name="ss-${port}"; fi
    
    jq "del(.inbounds[${index}])" "$CONFIG_FILE" > tmp.json && mv tmp.json "$CONFIG_FILE"
    jq "del(.\"$tag\")" "$META_FILE" > tmp_meta.json && mv tmp_meta.json "$META_FILE"
...
function main_menu() {
    ...
    echo "3) TUICv5 (自签证书)"
    echo "4) Shadowsocks (aes-256-gcm)"
    echo "5) SOCKS5"
    echo "----------------------------------------"
