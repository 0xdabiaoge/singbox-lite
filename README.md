...
### 2026.01.13
- **大幅增强 Hysteria2 端口跳跃功能**：引入应用层多端口监听模式，彻底解决 LXC/NAT 机器无法使用端口跳跃的问题，同时保留对大端口段的 iptables 支持。
- **重构批量创建功能**：优化交互流程（端口预告 -> SNI 自定义 -> 冲突检测），实现一次配置通用生效。
- **优化节点管理体验**：查看和删除列表自动隐藏 Hysteria2 的辅助分身节点，删除主节点时自动级联清理所有辅助端口，界面更清爽。
- **子脚本净化**：移除中转脚本中不必要的复杂逻辑，保持纯净。
