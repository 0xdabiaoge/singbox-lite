...
- 📦 **多协议支持** - 支持 9 种代理协议以及Argo临时隧道节点
- 🔗 **第三方节点导入** - 导入并适配第三方节点为本地监听
- 📋 **YAML 配置同步** - 自动生成 Clash 兼容配置（仅基础使用）
- 🔄 **节点管理** - 添加、删除、查看、修改节点
...
- **新增了Argo临时隧道节点的搭建，目前通过128M极限内存的Debian和Alpine系统测试，能实现singbox和Argo同时运行，仅为测试场景，实际使用预计占用会更高，尽量不要在≤128M内存的机器上使用Argo隧道。**
