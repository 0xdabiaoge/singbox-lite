...
**4、生成的节点链接导入到V2rayN即可正常使用。通过线路机的VLESS (Vision+REALITY)将流量转发到落地机，落地机访问后返回数据。**

**5、线路机转发脚本可单独安装使用，通过命令进行管理。与singbox脚本共存，搭建节点不冲突，只需要注意端口的使用即可。卸载的时候会弹出选择提示，保留线路机脚本服务则不删除singbox主程序，通过输出的命令进行管理。**

**6、新增Trojan+WS+TLS节点协议的搭建，搭建方法和用法同Vless+WS+TLS**

**7、新增节点名称自定义功能，也可以回车使用默认节点名称。新增脚本更新和SingBox核心更新的功能**
