...
**1、内置SingBox路由规则 (Route Rules)转发，灵感来自[singbox-deploy](https://github.com/caigouzi121380/singbox-deploy)，支持多个落地SS、VLESS(TCP)节点转发，感谢大佬提供的思路！**
...
**1、重构中转/落地功能为进阶功能，采用子脚本（advanced_relay.sh）进行统一管理，主菜单选择12) 进阶功能 (落地/中转配置)自动拉取子脚本。目前仅支持Shadowsocks (aes-256-gcm，2022-blake3-aes-128-gcm)、VLESS(TCP)这两种落地协议，搭配主脚本的节点搭建使用**

**2、中转入口协议采用VLESS (Vision+REALITY)、Hysteria2、TUICv5这三种协议，基本上满足绝大部分软件需求，机器线路好的情况下更推荐使用VLESS (Vision+REALITY)**

**3、用法：主脚本搭建SS或者VLESS(TCP)协议，主菜单选择12) 进阶功能 (落地/中转配置)进入子脚本管理界面，落地机选择1) 落地机配置 (VLESS-TCP / Shadowsocks)，选择已经搭建好的SS或者VLESS(TCP)协议，会输出一段加密Token，将这个Token复制。进入中转机，选择2) 中转机配置 (导入 Token)，将落地机生成的Token粘贴进去，选择中转协议，最后会输出中转节点链接。**

**4、中转节点和主脚本搭建的节点完全错开管理，互不影响，只需要注意端口的使用即可。**
