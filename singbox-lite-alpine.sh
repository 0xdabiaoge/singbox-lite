...
function get_listen_port() {
    while true; do
        read -p "请输入监听端口 (e.g., 443): " listen_port
...
# --- 主菜单 ---
function main_menu() {
    # FIX: 检查服务文件是否存在，而不是只检查二进制文件
    if [ ! -f /usr/local/bin/sing-box ] || [ ! -f /etc/init.d/sing-box ]; then
        echo "首次运行或安装不完整，正在执行初始化..."
        install_sing_box
    ...
    clear
    echo "sing-box Alpine 超精简安装脚本 (v5.5)"
    echo "=========================================="
    echo -e "请选择要搭建的节点类型:"
    echo -e " ${CYAN}1)${NC} VLESS (TCP, 无TLS)"
    ...
    echo -e " ${YELLOW}9)${NC} 卸载 sing-box (并自毁脚本)"
