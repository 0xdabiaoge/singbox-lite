...
_add_node_to_relay_yaml() {
    ...
    # 添加到 proxies 数组
    ${YQ_BINARY} eval ".proxies |= . + [${proxy_json}] | .proxies |= unique_by(.name)" -i "$RELAY_CLASH_YAML"
    # 添加到代理组
    ${YQ_BINARY} eval '.proxy-groups[] |= (select(.name == "中转节点") | .proxies |= . + ["'${proxy_name}'"] | .proxies |= unique)' -i "$RELAY_CLASH_YAML"
}
...
_remove_node_from_relay_yaml() {
    ...
    ${YQ_BINARY} eval 'del(.proxies[] | select(.name == "'${proxy_name}'"))' -i "$RELAY_CLASH_YAML"
    ${YQ_BINARY} eval '.proxy-groups[] |= (select(.name == "中转节点") | .proxies |= del(.[] | select(. == "'${proxy_name}'")))' -i "$RELAY_CLASH_YAML"
}
