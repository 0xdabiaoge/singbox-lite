...
_add_node_to_relay_yaml() {
    ...
    ${YQ_BINARY} eval '.proxy-groups[] |= (select(.name == "中转节点") | .proxies += ["'${proxy_name}'"] | .proxies |= unique)' -i "$RELAY_CLASH_YAML" 2>/dev/null
...
_remove_node_from_relay_yaml() {
    ...
    # 删除节点
    ${YQ_BINARY} eval 'del(.proxies[] | select(.name == "'${proxy_name}'"))' -i "$RELAY_CLASH_YAML" 2>/dev/null
    ${YQ_BINARY} eval '.proxy-groups[] |= (select(.name == "中转节点") | .proxies |= del(.[] | select(. == "'${proxy_name}'")))' -i "$RELAY_CLASH_YAML" 2>/dev/null
