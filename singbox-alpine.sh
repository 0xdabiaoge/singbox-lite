...
function vless_tcp_install() {
    ...
    local inbound=$(jq -n --arg t "$tag" --arg p "$port" --arg u "$uuid" \
        '{"type":"vless","tag":$t,"listen":"::","listen_port":($p|tonumber),"users":[{"uuid":$u}]}')
        ...
        case "$type" in
            "vless")
                ...
                if [[ $(echo "$node" | jq -r '.tls.reality.enabled') == "true" ]]; then
                    ...
                    url="vless://${uuid}@${display_ip}:${port}?encryption=none&security=none&type=tcp#$(url_encode "$tag")"
                fi
                ;;
            "hysteria2")
...
function main_menu() {
    ...
    echo -e " ${CYAN}2)${NC} VLESS (TCP)"
    echo -e " ${CYAN}3)${NC} Hysteria2 "
    echo -e " ${CYAN}4)${NC} TUICv5 "
    echo -e " ${CYAN}5)${NC} Shadowsocks (aes-256-gcm)"
    echo -e " ${CYAN}6)${NC} SOCKS5"
    echo "--- 管理选项 ---"
    echo -e " ${YELLOW}7)${NC} 查看节点分享链接"
    echo -e " ${YELLOW}8)${NC} 管理节点 (删除节点)"
