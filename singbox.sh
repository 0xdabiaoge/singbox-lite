...
# 系统特定变量
INIT_SYSTEM="" # 将存储 'systemd' 或 'openrc'
...
_detect_init_system() {
    if [ -f "/sbin/openrc-run" ]; then
        ...
        _error "无法识别的 Init 系统。本脚本仅支持 systemd 和 OpenRC。"
        exit 1
    fi
    _info "检测到 Init 系统为: ${INIT_SYSTEM}"
}
...
_install_dependencies() {
    ...
    if command -v apk &>/dev/null; then
        pm="apk"
        required_pkgs="bash coreutils procps ${required_pkgs}"
...
_add_hysteria2() {
    ...
    local inbound_json=$(jq -n --arg t "$tag" --arg p "$port" --arg pw "$password" --arg op "$obfs_password" \
        '{"type":"hysteria2","tag":$t,"listen":"::","listen_port":($p|tonumber),"users":[{"password":$pw}],"tls":{"enabled":true,"alpn":["h3"],"certificate_path":"'${SINGBOX_DIR}'/cert.pem","key_path":"'${SINGBOX_DIR}'/private.key"}} | if $op != "" then .obfs={"type":"salamander","password":$op} else . end')
    _atomic_modify_json "$CONFIG_FILE" ".inbounds += [$inbound_json]" || return 1
...
_add_tuic() {
    ...
    local tag="tuic-in-${port}"; local name="TUICv5-${port}"; local display_ip="$server_ip"; [[ "$server_ip" == *":"* ]] && display_ip="[$server_ip]"

    local inbound_json=$(jq -n --arg t "$tag" --arg p "$port" --arg u "$uuid" --arg pw "$password" \
        '{"type":"tuic","tag":$t,"listen":"::","listen_port":($p|tonumber),"users":[{"uuid":$u,"password":$pw}],"congestion_control":"bbr","tls":{"enabled":true,"alpn":["h3"],"certificate_path":"'${SINGBOX_DIR}'/cert.pem","key_path":"'${SINGBOX_DIR}'/private.key"}}')
    _atomic_modify_json "$CONFIG_FILE" ".inbounds += [$inbound_json]" || return 1
...
_add_shadowsocks_menu() {
    ...
    read -p "请输入监听端口: " port; [[ -z "$port" ]] && _error "端口不能为空" && return 1
    local tag="${name_prefix}-in-${port}"; local name="${name_prefix}-${port}"; local display_ip="$server_ip"; [[ "$server_ip" == *":"* ]] && display_ip="[$server_ip]"
...
_add_socks() {
    ...
    read -p "请输入密码 (默认随机): " password; password=${password:-$(${SINGBOX_BIN} generate rand --hex 16)}
    local tag="socks-in-${port}"; local name="SOCKS5-${port}"; local display_ip="$server_ip"; [[ "$server_ip" == *":"* ]] && display_ip="[$server_ip]"
    ...
    jq -c '.inbounds[]' "$CONFIG_FILE" | while read -r node; do
        ...
        case "$type" in
            "vless")
                ...
                ;;
            "hysteria2")
                local pw=$(echo "$node" | jq -r '.users[0].password'); local meta=$(jq -r --arg t "$tag" '.[$t]' "$METADATA_FILE");
                local op=$(echo "$meta" | jq -r '.obfsPassword')
                local obfs_param=""; [[ -n "$op" && "$op" != "null" ]] && obfs_param="&obfs=salamander&obfs-password=${op}"
                url="hysteria2://${pw}@${display_ip}:${port}?sni=www.microsoft.com&insecure=1${obfs_param}#$(_url_encode "$tag")"
                ;;
            "tuic")
                local uuid=$(echo "$node" | jq -r '.users[0].uuid'); local pw=$(echo "$node" | jq -r '.users[0].password')
                url="tuic://${uuid}:${pw}@${display_ip}:${port}?sni=www.microsoft.com&alpn=h3&congestion_control=bbr&udp_relay_mode=native&allow_insecure=1#$(_url_encode "$tag")"
                ;;
            "shadowsocks")
            ...
            "socks")
                ...
                _info "  类型: SOCKS5, 地址: $server_ip, 端口: $port, 用户: $u, 密码: $p"
                ;;
        esac
...
_main_menu() {
    while true; do
        clear
        echo "===================================================="
        _info "      sing-box 管理面板 ${SCRIPT_VERSION}"
        echo "===================================================="
        _info "【节点管理】"
        echo "  1) 添加节点"
        echo "  2) 查看节点分享链接"
