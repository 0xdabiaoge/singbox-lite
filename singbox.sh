...
_add_vless_ws_tls() {
    read -p "请输入用于客户端连接的域名 (CF上对应IP记录的二级域名): " domain
    [[ -z "$domain" ]] && _error "域名不能为空" && return 1
...
_view_nodes() {
    ...
    jq -c '.inbounds[]' "$CONFIG_FILE" | while read -r node; do
        ...
        local proxy_name_to_find=$(${YQ_BINARY} eval '.proxies[] | select(.port == '${port}' or .port == 443) | .name' ${CLASH_YAML_FILE} | grep "${type^^}" | head -n 1)
        ...
        case "$type" in
            "vless")
                ...
                if [ "$transport_type" == "ws" ]; then
                    ...
                    # 在分享链接中，server地址应为域名，即sni
                    url="vless://${uuid}@${sni}:${client_port}?encryption=none&security=tls&type=ws&host=${sni}&path=${encoded_path}#$(_url_encode "$tag")"
...
_show_add_node_menu() {
    ...
    case $choice in
        ...
        3) _add_vless_tcp; action_result=$? ;;
        4) _add_hysteria2; action_result=$? ;;
        5) _add_tuic; action_result=$? ;;
        6) _add_shadowsocks_menu; action_result=$? ;;
        7) _add_socks; action_result=$? ;;
        0) return ;;
        *) _error "无效输入，请重试。" ;;
    esac
