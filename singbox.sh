...
function manage_nodes() {
    ...
    local index=$((num - 1))
    local tag_to_del=$(jq -r ".inbounds[${index}].tag" "$CONFIG_FILE")
    local type_to_del=$(jq -r ".inbounds[${index}].type" "$CONFIG_FILE")
    local port_to_del=$(jq -r ".inbounds[${index}].listen_port" "$CONFIG_FILE")
    
    # --- FIX: 根据节点类型，正确生成要在 clash.yaml 中删除的节点名称 ---
    local name_to_del
    case "$type_to_del" in
        "vless") name_to_del="vless-reality-${port_to_del}" ;;
